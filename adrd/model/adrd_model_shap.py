...
class ADRDModel(BaseEstimator):
    ...
    def __init__(self,
        ...
        self.clr_ratio = clr_ratio
        self.emb_droprate = emb_droprate
        self.name = name
        self._init_net()
        self.thresholds = torch.zeros(1)
        self.skip_embedding = {}
        for k, info in self.src_modalities.items():
            self.skip_embedding[k] = False

    def linear_warmup(self, epoch, warmup_epochs=20):
    ...
    def fit(self, x_trn, x_vld, y_trn, y_vld, img_train_trans=None, img_vld_trans=None, img_mode=0) -> Self:
        ...
        if self.wandb_ == 1 and self.rank == 0:
            wandb.init(
                ...
                project="LMTDE",
                # name=f"GM",# name=f"hd{self.nhead}_nlay{self.num_encoder_layers}_lr{self.lr}",
                name=self.name,
        ...
        self.cos_scheduler = self._init_scheduler(self.optimizer)
        ...
        self.warmup_scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer, lr_lambda=lambda epoch: self.linear_warmup(epoch))
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            # met_trn = self.train_one_epoch(ldr_trn, epoch, freeze_otr=freeze_otr)
    ...
    def find_best_threshold(self, score, true_label):
        """
        输入:
            score: 模型输出的概率值 (numpy数组或列表)
            true_label: 真实标签 (numpy数组或列表)
        返回:
            best_threshold: 最佳阈值
            best_balanced_acc: 最佳阈值对应的 Balanced Accuracy
        """
        # 将输入转换为 numpy 数组
    ...
    def compute_metrics_ddp(self, scores_list, y_true_list, y_mask_list, losses_list, training = True):
        ...
        if dist.get_rank() == 0:
            ...
            thresholds = []
            if training:
                n_label = scores.shape[-1]
                split_scores = [scores[:, i] for i in range(n_label)]
                split_ytrue = [y_true[:, i] for i in range(n_label)]
                for i in range(n_label):
                    TH, _ = self.find_best_threshold(split_scores[i], split_ytrue[i])
                    thresholds.append(TH)
                self.thresholds = torch.tensor(thresholds).to(self.device)

            thresholds = self.thresholds
            TH_map = thresholds.unsqueeze(0).expand(scores.shape[0], -1)
            '''
            TH_map = 0
    ...
    def train_one_epoch(self, ldr_trn, epoch, freeze_otr=False):
        ...
        # set self.cos_scheduler according freezing or not
        # restart cosine scheduler
        epoch = epoch - 20
        if epoch < 0:
            self.warmup_scheduler.step(epoch + 20)
        else:
            if epoch == 192:
                self.cos_scheduler = self._init_scheduler(self.optimizer, T_mult=2)
            if epoch >= 192:
                self.cos_scheduler.step(epoch - 192)
        # self.optimizer.param_groups[1]['lr'] = 0.0 if freeze_otr else self.lr
        # self.cos_scheduler.base_lrs = [group['lr'] for group in self.optimizer.param_groups]
        ...
        for n_iter, (x_batch, y_batch, mask, y_mask) in enumerate(ldr_trn):
            ...
            with torch.autocast(
                ...
                if epoch < 192: detach = True
                else: detach = False
                representation, outputs = self.net_(x_batch, mask, skip_embedding=self.skip_embedding, detach=detach)
                ...
                for i, k in enumerate(self.tgt_modalities):
                    ...
                    # print(f"------------{i}--------")
                    # assert(not torch.isnan(outputs[k]).any())
                    # assert(not torch.isnan(loss_task).any())
                    msk_loss_task = loss_task * y_mask[k]
                    msk_loss_mean = msk_loss_task.sum() / ((torch.sum(y_mask[k]))+1e-12)
                    loss += msk_loss_mean
                    # print(f"\n------------------- {loss} ---------")
                ...
                for i, k in enumerate(self.tgt_modalities): 
                    ...
                    w = 1
                    nce_loss += nn.dual_temperature_loss_func(rept, y_k, y_mask_k, self.clr_temp)
            ...
            if self._amp_enabled:
                ...
                gamma = self.clr_ratio # if epoch > 256 else 0
                # assert(gamma == 0)
    ...
    def load(self, filepath: str, map_location: str = 'cpu', img_dict=None) -> None:
        ...
        #if os.environ.get('SHAP') is not None and os.environ.get('SHAP').lower in ['true', '1', 't', 'yes']:
        if True:
            state_dict.pop('src_modalities')
            state_dict.pop('tgt_modalities')
            ic("SHAP 运行中, loading the model with SHAP parameters...")
        else:
        ...
        ic.enable()
        ic("before")
        self.net_ = Transformer(self.src_modalities, self.tgt_modalities, self.d_model, self.nhead, self.num_encoder_layers, self.num_decoder_layers, self.device, self.img_net, self.imgnet_layers, self.img_size, self.patch_size, self.imgnet_ckpt, self.train_imgnet, self.fusion_stage, emb_path=self.emb_path, emb_droprate = self.emb_droprate)
        ic("222222222222")
    ...
    def _init_net(self):
        ...
        self.net_ = nn.Transformer(
            ...
            imgnet_layers = self.imgnet_layers, 
    ...
    def _init_optimizer(self):
        ...
        other_params = [param for param in self.net_.parameters()]
    ...
    def _init_scheduler(self, optimizer, T_mult=1):
        """ ... """       
