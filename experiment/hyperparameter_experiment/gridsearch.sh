...
for temp in 0.2 0.1 1 0.03; do
    for clr_ratio in 1 3 0.5; do
        ckpt_path="${prefix}/dev/ckpt/T=${temp};gamma=${clr_ratio}.pt"
        torchrun --nproc_per_node=6 "${prefix}/dev/train.py" --data_path $data_path --train_path $train_path --vld_path $vld_path --test_path $test_path --cnf_file $cnf_file --ckpt_path $ckpt_path --d_model 256 --nhead 1 \
                        ...
                        --contrastive --name "gridsearch:T=${temp};gamma=${clr_ratio}"
    done
done
